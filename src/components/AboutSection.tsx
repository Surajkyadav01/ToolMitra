      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-20">
          <div className="lg:col-span-5 space-y-5">
            <div className="text-indigo-655 dark:text-cyan-400 font-bold text-xs uppercase tracking-widest font-mono">
              {t('about_tag', 'About ToolMitra')}
            </div>
            ...
            {choicePoints.map((point, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2.5">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-50 to-indigo-100/50 dark:from-slate-800 dark:to-slate-850 text-indigo-650 dark:text-cyan-400 flex items-center justify-center shrink-0 shadow-sm group-hover:scale-115 transition-transform duration-300">
                  ...
                  <h4 className="font-sans font-bold text-slate-900 dark:text-white text-base group-hover:text-indigo-600 dark:group-hover:text-cyan-400 transition-colors">
                    {t(`cp_${index}_title`, point.title)}
                  </h4>
                </div>
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-normal">
                  {t(`cp_${index}_desc`, point.description)}
                </p>
              </div>
            ))}
          </div>
        ...
        <div id="faq-block" className="max-w-4xl mx-auto space-y-6">
          <div className="text-center space-y-2 mb-10">
            ...
            <p className="text-sm text-slate-400">
               {t('about_faq_subtitle', 'Everything you need to understand regarding operations, browser storage, and file limits.')}
            </p>
          </div>
            ...
            {DEMO_FAQS.map((faq, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span>{t(`faq_${index}_question`, faq.question)}</span>
                  ...
                  {isOpen && (
                    <div className="px-5 pb-5 pt-1.5 text-sm text-slate-600 dark:text-slate-400 leading-relaxed font-normal border-t border-slate-100 dark:border-slate-850/30">
                      {t(`faq_${index}_answer`, faq.answer)}
                    </div>
                  )}
                </div>
              );
